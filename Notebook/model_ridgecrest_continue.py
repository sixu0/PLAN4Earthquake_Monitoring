...
class Conv_downsample(nn.Module):
    def __init__(self, in_ch, out_ch, ifpool=False, poolstride=2):
        ...
        layers = [
            ...
            nn.ReLU(),
            nn.Conv1d(in_channels=out_ch, out_channels=out_ch,
                      kernel_size=7, stride=1, padding=3),
            nn.BatchNorm1d(out_ch),
            nn.ReLU(),
        ]
...
class Conv_upsample(nn.Module):
    def __init__(self, in_ch, out_ch, poolstride=2):
        ...
        layers = [
            nn.ConvTranspose1d(in_channels=in_ch, out_channels=out_ch, kernel_size=7,
...
class Main_GCNN(torch.nn.Module):
    def __init__(self, act_layer='Trans'):
        ...
        self.gd1 = Conv_downsample(
            3, 16, ifpool=True, poolstride=4)  # 3072 768
        self.gd2 = Conv_downsample(
            16, 32, ifpool=True, poolstride=4)  # 768 192
    ...
    def forward(self, x, station_loc, batch, edge_index):
        ...
        aux_out_offset, aux_out_depth, x_temp = self.aux_head(
            out, station_loc, batch, edge_index)
        dtp, dts = self.time_head(
            aux_out_offset, aux_out_depth, batch, edge_index)
...
class SIMConv(MessagePassing):
    ...
    def message(self, x_j: Tensor, x_i: Tensor, index: Tensor, ptr: OptTensor,
        ...
        alpha = torch.exp(-(x_i[:, self.start_feature:self.end_feature] -
                          x_j[:, self.start_feature:self.end_feature]).abs().sum(dim=-1))
